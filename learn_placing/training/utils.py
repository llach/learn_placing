...
def get_dataset_loaders(name, target_type="world2object", matrix=False, train_ratio=0.8, batch_size=8, shuffle=True):
    ...
    X = torch.Tensor(X)
    Y = torch.Tensor(Y)
    if matrix: Y = compute_rotation_matrix_from_quaternion(Y)
